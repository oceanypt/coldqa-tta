  ...
  for step, train_batch in enumerate(train_batchs):
    ...
    with torch.no_grad():
      train_inputs = {
        ...
        "token_type_ids": None if args.model_type in ["xlm", "distilbert", "xlm-roberta"] else train_batch[2],
      }
      example_indices = train_batch[3]

      # XLNet and XLM use more arguments for their predictions
      if args.model_type in ["xlnet", "xlm"]:
        train_inputs.update({"cls_index": train_batch[4], "p_mask": train_batch[5]})
      if args.model_type == "xlm":
...
def main():
  ...
  results = {}
  all_res = []
  if args.do_OIL and args.local_rank in [-1, 0]:
    if args.do_train:
      logger.info("Loading checkpoints saved during training for evaluation")
      checkpoints = [args.output_dir]
      if args.eval_all_checkpoints:
